...
# Open Science experimental project developed by MPC-ES team with the aim of encouraging the creation of similar works.
...
# Samantha uses llama.cpp via llama-cpp-python to load and run Large Language Models (LLMs) in UGGF format.
# 1) To run llama.cpp, download and install Microsoft Visual Studio Community: https://visualstudio.microsoft.com/pt-br/downloads/?cid=learn-navbar-download-cta
...
# 2) To read aloud the text generated by the models in high quality, use Microsoft Edge browser's online TTS feature (keyboard shortcut: Shift + Ctrl + U).
...
#                FOURTH LOOP - TOKEN GENERATION      <<<<<<<<<<<< 4TH LOOP
#                   FAST MODE
#                   NORMAL MODE
#                   LEARNING MODE
#                   TEXT TO SPEECH
...
# VSCODE DEBUG CODE EXPLORER TIPS
...
# CTRL + K -> CTRL + 0 = COLLAPSE ALL VSCODE ITEMS
# CTRL + K -> CTRL + J = EXPAND ALL VSCODE ITEMS

# STEPS TO ACTIVATE JUPYTERRLAB VIRTUAL ENVIRONMENT WITH CONDA (e.g. TO INSTALL MODULES WITHIN)
# (samantha) full-path-to-current-directory>
# (samantha) full-path-to-current-directory> conda activate <full-path-to-current-directory>\miniconda3\envs\jupyterlab
...
import gradio as gr               # Import Gradio to create a user-friendly web interface for interacting with AI models. This library simplifies the process of deploying models with a front-end that can accept inputs and display outputs (Front-End Interface): https://www.gradio.app/
from llama_cpp import Llama       # Import Llama class from llama-cpp-python module to load Large Language Model (LLM) files in UGGF format. This is the back-end component responsible for handling the model's processing (Back-End Model Loading): https://llama-cpp-python.readthedocs.io/en/latest/
import pyttsx3                    # Import pyttsx3 to convert text to speech using SAPI5 voices installed on the computer, enabling textual information to be audibly spoken out loud (Text-to-Speech): https://github.com/nateshmbhat/pyttsx3
import pandas as pd               # Import pandas to create DataFrames for visualization purposes, particularly useful for generating bar plots from token data (Data Analysis and Visualization): https://pandas.pydata.org/
import numpy as np                # Import numpy for numerical computing and data manipulation, providing support for large, multi-dimensional arrays and matrices, along with a collection of mathematical functions (Numerical Computing): https://numpy.org/devdocs/index.html
...
click = pygame.mixer.Sound(fr"{DIRETORIO_LOCAL}\click.mp3") # Load and store a sound object for a Gradio interface click event (click sound), sourced from another local file path, intended to be used for interactive elements like buttons.
...
def read_aloud_fn(text: str):
    ...
    """
    try:                                        # Attempt to delete previous 'resposta.mp3' to allow the creation of a new audio file with the latest text
        os.remove('resposta.mp3')
    except:                                     # If an error occurs during deletion, do nothing (pass)
        pass
    engine.save_to_file(text, 'resposta.mp3')   # Save the text to speech audio file named 'resposta.mp3' in current directory
...
# The code below checks if the string 'portuguese' is present in the lowercase version of the 'name' attribute of the 'voices' object.
...
# The code is designed to be bilingual, with English and Portuguese configurations provided. 
...
llama_cpp_python_version = '0.3.16' # This variable must be updated when llama_cpp_python module is upadated in Samantha
samantha_version = '0.18.0'         # This variable must be updated when Samantha is updated
language = {
            # PORTUGUESE
            'pt': {
                # TITLE ===============================================
                'title': 'Samantha Interface Assistant',
                'subtitle_1': f'Interface Experimental Desenvolvida para Testar Modelos de Inteligência Artificial de Código Aberto (<a href="https://github.com/controlecidadao/samantha_ia">Versão {samantha_version}</a>)',
                'warning': 'ATENÇÃO',
            ...
            'en': {
                # TITLE ===============================================
                'title': 'Samantha Interface Assistant',
                'subtitle_1': f'Experimental Interface Designed to Test Open Source Artificial Intelligence Models (<a href="https://github.com/controlecidadao/samantha_ia">Version {samantha_version}</a>)',
                'warning': 'WARNING',
...
# The following code prompts the user to select their preferred language for the interface interaction, with the options Portuguese or English
...
if 'portuguese' in voices[0].name.lower(): # Check if the name of the voice contains the substring 'portuguese'. If it does, proceed to offer language selection in Portuguese.
    ...
    try:                                    # try/except block to catch exceptions that may occur when reading input (e.g., EOFError)
...
def get_downloads_path():
    ...
    if os.path.exists(temp):                            # Check if the "Downloads" directory exists at the constructed path
        ...
        return None                                     # If the Downloads directory does not exist, return None to indicate that the path is not valid
...
# Define global variables to store the state of the interface and the AI model, including the user's input, the model's output, and various settings and configurations.
# These variables are used to manage the interaction between the user, the AI model, and the interface elements, ensuring a smooth and responsive user experience.
...
# The code bellow reads two text files containing templates for user and system prompts, respectively. 
        ...
        while True:
            ...
            if last_model != model or llm == '' or loop_models > 1: # Check if model changed or was unloaded. If so, load new model. Force unload model if loop_model > 1
                ...
                try:                                                # Try to load model                    
                    ...
                    #     yield 'No space left on device. Try downloading a smaller model.'
...
def clean_output():                 # Clear Output button: clear output text
    ...
    """
    This function clears all variables and files with text and audio generated by user and Assistant.
    """
    ...
    with open('last_user_prompt.txt', 'w', errors='ignore') as f: # Delete content of the file 'partial_text.txt'
        pass

    with open('last_system_prompt.txt', 'w', errors='ignore') as f: # Delete content of the file 'partial_text.txt'
        pass

    with open('last_previous_response.txt', 'w', errors='ignore') as f: # Delete content of the file 'partial_text.txt'
        pass
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            gr.HTML("""<ul>
                        ...
                        <li><a href="https://chat.lmsys.org/">ChatArena Leaderboard</a></li>
                        <li><a href="https://livebench.ai/#/">LiveBench Leaderboard</a></li>
            ...
            gr.HTML("""<ul>
                    ...
                    <li><a href="https://selenium-python.readthedocs.io/index.html">Selenium</a> <a href="https://pypi.org/project/webdriver-manager/">Webdriver Manager</a></li>
                    </ul>""")
        ...
        with gr.Column(scale=1):
            ...
            with gr.Row():
                gr.HTML("""<ul>
                        ...
                        <a href="https://dashboard.cohere.com/welcome/login">Cohere</a>
                        <a href="https://space.reka.ai/sign-in">Reka</a>
